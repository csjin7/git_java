...
public class PerformanceEx {
	...
	public static void main(String[] args) throws Exception {
		Path from = Paths.get("src/chap19/house.jpg");
		
		Path to1 = Paths.get("src/chap19/house2.jpg");
		Path to2 = Paths.get("src/chap19/house3.jpg");

		long size = Files.size(from);
		
		FileChannel fileChannel_from = FileChannel.open(from);
		FileChannel fileChannel_to1 = 
			FileChannel.open(to1, EnumSet.of(StandardOpenOption.CREATE, StandardOpenOption.WRITE));
		FileChannel fileChannel_to2 = 
			FileChannel.open(to2, EnumSet.of(StandardOpenOption.CREATE, StandardOpenOption.WRITE));
